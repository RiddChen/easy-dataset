...
import MultiTurnDialog from './components/MultiTurnDialog';
...
export default function ImagesPage() {
  ...
  const [multiTurnDialogOpen, setMultiTurnDialogOpen] = useState(false);
  ...
  const handleGenerateMultiTurn = image => {
    setSelectedImage(image);
    setMultiTurnDialogOpen(true);
  };
  ...
  const handleBatchGenerateMultiTurn = async () => {
    if (!selectedModel) {
      toast.error('Select a vision model first');
      return;
    }

    if (selectedModel.type !== 'vision') {
      toast.error('Current model is not a vision model');
      return;
    }
    ...
    try {
      ...
      if (!String(config.imageMultiTurnFirstQuestion || '').trim()) {
        toast.error('Set the image multi-turn first prompt in Project Settings -> Task Settings');
        return;
      }

      const response = await axios.post(`/api/projects/${projectId}/tasks`, {
        taskType: 'image-multi-turn-generation',
        modelInfo: selectedModel,
        language: i18n.language,
        note: selectedIds.length > 0 ? { imageIds: selectedIds } : {}
      });

      if (response.data.code === 0) {
        toast.success(selectedIds.length > 0 ? 'Batch image multi-turn task created for selected images' : 'Batch image multi-turn task created');
        router.push(`/projects/${projectId}/tasks`);
        return;
      }

      toast.error(response.data.error || 'Failed to create image multi-turn task');
    } catch (error) {
      console.error('Failed to create image multi-turn task:', error);
      toast.error(error.response?.data?.error || 'Failed to create image multi-turn task');
    }
  };
  // 确认创建自动提取任务
    ...
    <Container maxWidth="xl" sx={imageStyles.pageContainer}>
      ...
      <Box sx={imageStyles.header}>
        ...
        <Box sx={imageStyles.headerActions}>
          ...
          <Button
            variant="outlined"
            startIcon={<AutoAwesomeIcon />}
            onClick={handleBatchGenerateMultiTurn}
            sx={imageStyles.actionButton}
          >
            {selectedIds.length > 0 ? `Batch 2-turn (${selectedIds.length})` : 'Batch 2-turn'}
          </Button>
          <Button
            variant="contained"
      ...
      {loading ? (
        ...
        <ImageGrid
          ...
          onGenerateMultiTurn={handleGenerateMultiTurn}
      ...
      <MultiTurnDialog
        open={multiTurnDialogOpen}
        projectId={projectId}
        image={selectedImage}
        onClose={() => setMultiTurnDialogOpen(false)}
        onSuccess={conversation => {
          ...
          router.push(`/projects/${projectId}/image-conversations`);
        }}
      />
